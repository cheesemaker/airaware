...
extension AwairService : WKNavigationDelegate {
	...
	public func promptForUserLogin(viewController : UIViewController, _ completion : @escaping(Bool)->Void) {
		...
		let path = AwairService.loginPath + "?state=12345678&response_type=code&client_id=\(self.clientID)&redirect_uri=\(self.redirectURL)"
	...
	func refreshAuthToken(_ completion: @escaping(Bool)->Void) {
		...
		let request = UUHttpRequest(url: AwairService.tokenExchangePath)
